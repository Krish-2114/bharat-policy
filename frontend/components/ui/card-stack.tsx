...
export function CardStack<T extends CardStackItem>({
  ...
  return (
    <div
      className={cn("w-full relative py-0", className)}
    ...
    >
      ...
      <div
        ...
        style={{ height: Math.max(380, cardHeight + 40) }}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
        {/* background wash / spotlight */}
        <div
          className="pointer-events-none absolute inset-x-0 top-12 mx-auto h-48 w-[60%] rounded-full bg-blue-500/5 blur-[80px]"
          aria-hidden="true"
        />
              ...
              return (
                <motion.div
                  ...
                  className={cn(
                    ...
                    "will-change-transform select-none bg-slate-900",
                    isActive
                      ? "cursor-grab active:cursor-grabbing ring-2 ring-cyan-500 shadow-[0_0_40px_rgba(6,182,212,0.15)]"
                      : "cursor-pointer opacity-30 hover:opacity-50 transition-opacity blur-[2px]",
                  )}
                  style={{
                    width: cardWidth,
                    height: cardHeight,
...
function DefaultFanCard({ item, active }: { item: CardStackItem; active: boolean }) {
  return (
    <div className="relative h-full w-full bg-slate-900 group">
      ...
      <div className="absolute inset-0">
        {item.imageSrc ? (
          <img
            src={item.imageSrc}
            alt={item.title}
            className="h-full w-full object-cover opacity-40 transition-opacity duration-700 group-hover:opacity-60"
            draggable={false}
            loading="eager"
          />
        ) : (
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-900/80 to-slate-900/10" />
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-8">
        ...
        {item.subtitle && (
          <div className="text-sm md:text-base font-medium text-cyan-300 mb-4">
            {item.subtitle}
          </div>
        )}
        {item.description ? (
          <div className={cn(
            "text-sm md:text-base text-slate-300 leading-relaxed font-light transition-all duration-500",
